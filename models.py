...
class Pedido(models.Model):
    ...
    def quantidade_total(self):
        itens_pedidos = ItemPedido.objects.filter(pedido__id=self.id)
        quantidade = sum([item.quantidade for item in itens_pedidos])
        return quantidade

    @property
    def preco_total(self):
        itens_pedidos = ItemPedido.objects.filter(pedido__id=self.id)
        preco = sum([item.preco_total for item in itens_pedidos])
        return preco
class ItemPedido(models.Model):
    ...
    def __str__(self):
        return f"Id pedido: {self.pedido.id} - Produto: {self.item_estoque.produto}, {self.item_estoque.tamanho}, {self.item_estoque.cor.nome}"
